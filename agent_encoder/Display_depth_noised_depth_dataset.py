...
用于 vanilla VAE 训练
在加载数据集之前，使用
该方法是数据集准备 本项目使用该方法实现数据集加载           深度图和 有噪声的深度图   没有碰撞图
1. 读取深度图和碰撞图
2. 归一化深度图到[0, 255]范围
3. 添加噪声，增加数据增强
...
# 数据集类
class NoiseDepthDataset(Dataset):
    def __init__(self, depths_folder, transform=preprocess_image, return_file_name=False):
    ...
    # 增加数据增强 主要是图像翻转
    def augment_image(self, depth, colls):
        """随机翻转图像"""
    ...
    def __getitem__(self, index):
        ...
        # 0.5的概率添加噪声
        if np.random.rand() < 0.5:
            depth_image_copy = self.add_depth_dependent_noise(depth_image_copy)
        else:
            depth_image_copy = depth_image_copy

        # 归一化深度图
        depth_image_copy = self.uint8_0_normalize(depth_image_copy)
        # 数据增强
        depth_image, depth_image_copy = self.augment_image(depth_image, depth_image_copy)
        # 应用转换为tensor
        if self.transform:
            depth_image = self.transform(depth_image)
            depth_image_copy = self.transform(depth_image_copy)

        if self.return_file_name:
            return depth_image_copy, depth_image, self.depth_files[index]
        return depth_image_copy, depth_image   # , self.depth_files[index]  # 返回文件名用于调试
...
if __name__ == "__main__":
    ...
    # 测试数据集和数据加载器   深度图和 有噪声的深度图   没有碰撞图
