...
from agent_encoder.Project_dataset_show import RGBDepthCollisionDataset

"""
用于 DC VAE 训练数据的展示  
注意此处 PureDepthCollisionDataset 没有添加噪声，但是导入的是 DepthCollisionDataset
在加载数据集之前，使用
该方法是数据集准备 本项目使用该方法实现数据集加载       深度图和碰撞图
1. 读取深度图和碰撞图
2. 归一化深度图到[0, 255]范围
3. 添加噪声，增加数据增强
...
# 数据集类
class PureDepthCollisionDataset(Dataset):
