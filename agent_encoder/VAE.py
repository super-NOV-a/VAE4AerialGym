...
# 该文件定义了一个VAE类，该类继承自nn.Module，vae_image_encoder.py中的VAEImageEncoder类使用了这个类
...
class ImgEncoder(nn.Module):
    ...
    def encode(self, img):
        ...
        x = x3_0.view(x3_0.size(0), -1)  # x = x3_0.reshape(x3_0.size(0), -1)  # x3_0.view(x3_0.size(0), -1)
...
class VAE(nn.Module):
    ...
    def __init__(self, input_dim=1, latent_dim=64, with_logits=False, inference_mode=False):
        ...
        self.encoder = ImgEncoder(input_dim=self.input_dim, latent_dim=self.latent_dim)   # dronet  encoder
