...
"""
用于展示 原始深度图，由于没有数据增强不适用于训练模型
在加载数据集之前，使用
该方法是数据集准备 本项目使用该方法实现数据集加载      纯深度图和深度图   没有碰撞图  没有数据增强等内容
1. 读取深度图和碰撞图
2. 归一化深度图到[0, 255]范围
3. 添加噪声，增加数据增强
...
# 数据集类
class DepthPureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.return_file_name:
            return depth_image, depth_image_copy, self.depth_files[index]
        return depth_image, depth_image_copy    # , self.depth_files[index]  # 返回文件名用于调试
...
if __name__ == "__main__":
    ...
    # 测试数据集和数据加载器   # 深度图和深度图   没有碰撞图  没有数据增强等内容
