...
if __name__ == "__main__":
    # 示例深度图像和边缘点
    depth_file = "/data_test/depths/depth_image_0.png"
    ...
    Colls_image = 25.5 * np.minimum(offset_depth, raycast_img)  # 在result2中计算碰撞图像也需要

    # plt 在一个窗口中显示depth_img和 Colls_image
    plt.figure(figsize=(12, 6))
    plt.subplot(1, 3, 1)
    plt.imshow(depth_img_copy, cmap='gray', vmin=0, vmax=255)
    ...
    plt.axis('off')
    plt.subplot(1, 3, 3)
    plt.imshow(Colls_image, cmap='gray')
